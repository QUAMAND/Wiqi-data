...
마인크래프트는 채팅을 제외한 [[:markdown: 01-1 컨텍스트](#/doc/2/01/1)]에 아무런 값도 작성하지 않으면 `0.5, 0.0, 0.5`으로 인식합니다.
```mcfunction
...
# 방향(Rotation)
일반적인 시선을 나타내는 값으로, 현재 [[:markdown: 01-1 컨텍스트](#/doc/2/01/1)]를 기준으로 시선을 지정합니다.
...
마인크래프트는 채팅을 제외한 [[:markdown: 01-1 컨텍스트](#/doc/2/01/1)]에 아무런 값도 작성하지 않으면 `0.0, 0.0`으로 인식합니다.
```mcfunction
# 무조건 명령 블록 혹은 함수에서 실행하십시오.
