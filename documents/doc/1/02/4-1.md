- [[:paper: 02-4 재귀함수](#/doc/1/02/4)] 글을 읽고 오는 것을 추천드립니다.
...
[[:paper: 02-4 재귀함수](#/doc/1/02/4)] 페이지 하단에 `y0.0`에서 `y1.0`까지 `dust` 입자를 표시하는 예시 구문을 확인할 수 있는데,
...
`execute` 명령어 인수 중, `as`/`facing`/`rotated` 등과 같은 [[:paper: 01-1 컨텍스트](#/doc/2/01/1)]는 `n` 개의 엔티티만큼 실행합니다.
